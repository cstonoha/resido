#' Adapted from baseq: Basic Sequence Processing Tool for Biological Data V0.0.2
#' baseq was developed by Developed by Ambu Vijayan and Dr. J. Sreekumar
#'
...
write.df_to_fasta <- function(df, output_dir = getwd()) {
  ...
  for (i in 1:nrow(df)) {
    header <- paste0(">", df$seq_name[i])
    sequence <- df$seq_aa[i]
