#' function for calculating count of a user specified amino acid
#' counts a user-specified amino acid in the sequence
#' output is a dataframe with the amino acid count for each sequence
#' @import stringr
#' @importFrom utils write.csv
#' @importFrom utils data

#' @param file a fasta file the contains the sequence name and amino acid sequence
#' @param amino_acid the amino acid of interest
...
resido_amino_acid_count <- function(file, amino_acid, sort_by = "seq_name", decreasing = TRUE, output_format = "dataframe"){
  ...
  #count number of specified amino acid
  single_amino_acid_count <- str_count(fasta_dataframe$seq_aa, amino_acid)

  #add amino acid count as a new column
  fasta_dataframe$amino_acid_count <- (single_amino_acid_count)
